...
class TrackInPlaylistRepositoryImpl(
    ...
    override suspend fun getTracksInPlaylist(playlist: Playlist): Flow<List<Track>> = flow {
        ...
        for (trackId in playlist.tracksRegister) {
            listOfTracks.add(
                trackInPlaylistDBConverter.map(
                    appDatabase.trackInPlaylists().getTrackInPlaylist(trackId)
    ...
    override suspend fun deleteTrackFromPlaylist(trackId: Int, playlist: Playlist): Playlist {
        ...
        appDatabase.playlistDao().updateTrackToPlaylist(
            playlistDBConverter.fromListToString(newRegister),
            playlist.innerId
        )
        val numberOfPlaylist =
            appDatabase.trackInPlaylists().getTrackInPlaylist(trackId)!!.numberOfPlaylists
