...
import com.example.playlistmaker.media.ui.playlists.pl_create.CreatePlaylistFragment
...
class PlayerTrackFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (requireArguments().getInt(INCOME_ID) == CRPL_SEARCH_ID || requireArguments().getInt(
                INCOME_ID
            ) == CRPL_MEDIA_ID
        ) {
            bottomSheetBehavior.state = BottomSheetBehavior.STATE_EXPANDED
        ...
        recyclerPlayerPlaylistAdapter.setOnClickListener(object :
            PlayerPlaylistsAdapter.OnClickListener {
            override fun onClick(position: Int, playlist: Playlist) {
                if (viewModel.addTrackToPlaylist(track, playlist)) {
                    Toast.makeText(
                        requireContext(),
                        getString(R.string.player_toast_already_added) + playlist.name,
                        Toast.LENGTH_SHORT
                    ).show()
                } else {
                    Toast.makeText(
                        requireContext(),
                        getString(R.string.player_toast_added) + playlist.name,
                        Toast.LENGTH_SHORT
                    ).show()
                    viewModel.getPlaylistList()
                    bottomSheetBehavior.state = BottomSheetBehavior.STATE_HIDDEN
    ...
    private fun backHandle() {
        ...
        when (requireArguments().getInt(INCOME_ID)) {
            ...
            PL_INFO_ID, CRPL_PLINFO_ID -> findNavController().popBackStack(
