package com.example.playlistmaker.media.data.repository.playlist
...
class PlaylistControlDBRepositoryImpl(
    ...
    override suspend fun getUpdatedPlaylist(playlistID: Int): Flow<Playlist?> = flow {
        val playlist = appDatabase.playlistDao().getUpdatedPlaylist(playlistID)
        emit(convertToPlaylistOne(playlist))
    }

    override suspend fun updatePlaylist(playlist: Playlist) {
        appDatabase.playlistDao()
            .updatePlaylist(playlist.name, playlist.description, playlist.artLink, playlist.innerId)
    }
    ...
    private fun convertToPlaylistOne(playlist: PlaylistEntity): Playlist {
        return playlistDBConverter.map(playlist)
    }

    private fun convertToPlaylistEntityOne(playlist: Playlist): PlaylistEntity {
        return playlistDBConverter.map(playlist)
    }
}